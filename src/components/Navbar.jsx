...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-[100] transition-all duration-500 ${scrolled ? "bg-white/90 backdrop-blur-xl border-b border-purple-100 py-3 shadow-sm" : "bg-transparent py-6"
      }`}>
      <div className="flex items-center justify-between px-6 md:px-12 max-w-7xl mx-auto">
        <Link to="/" className="flex items-center gap-3 z-[110] group">
          ...
          <span className="text-2xl font-black italic text-[#1e1b4b] tracking-tighter">TaxPal</span>
        </Link>

        <div className="hidden lg:flex items-center gap-10">
          <div className="flex gap-10 text-[11px] font-black text-slate-500 tracking-[0.2em] uppercase">
            {navLinks.map((link) => (
              <a key={link.name} href={link.path} className="hover:text-[#6d28d9] transition-colors relative group">
                {link.name}
          ...
          <div className="flex items-center gap-4 border-l border-purple-100 pl-8">
            {!isLoggedIn ? (
              <>
                <button onClick={() => navigate('/login')} className="text-[#1e1b4b] text-[13px] font-black uppercase tracking-widest hover:text-[#6d28d9] transition-colors px-4">
                  Sign In
                </button>
                <button onClick={() => navigate('/register')} className="bg-[#6d28d9] text-white px-7 py-3 rounded-2xl font-black text-[11px] uppercase tracking-widest shadow-xl shadow-purple-100 hover:bg-[#5b21b6] hover:-translate-y-0.5 transition-all">
                  Get Started
                </button>
              </>
            ) : (
              <div className="relative" ref={dropdownRef}>
                <button onClick={() => setIsProfileOpen(!isProfileOpen)} className="flex items-center gap-3 p-1.5 pr-4 rounded-2xl bg-white border border-purple-100 hover:border-purple-300 transition-all shadow-sm group">
                  <div className="w-9 h-9 rounded-xl bg-purple-50 flex items-center justify-center text-[#6d28d9] font-black group-hover:bg-[#6d28d9] group-hover:text-white transition-colors">
                    JD
                  </div>
        ...
        <button className="lg:hidden p-3 rounded-2xl bg-purple-50 text-[#6d28d9] transition-all z-[110]" onClick={() => setIsOpen(!isOpen)}>
          ...
          <>
            ...
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} className="fixed right-0 top-0 h-full w-[80%] max-w-sm bg-white lg:hidden z-[105] shadow-2xl flex flex-col p-8">
              <div className="mt-20 flex flex-col gap-6 flex-1">
                {navLinks.map((link) => (
                  <a key={link.name} href={link.path} className="text-3xl font-black text-[#1e1b4b] italic hover:text-[#6d28d9] transition-colors">{link.name}</a>
                ))}
              </div>
              <div className="mt-auto space-y-3">
                {isLoggedIn ? (
                  <>
                    <button onClick={() => navigate('/dashboard')} className="w-full flex items-center justify-center gap-3 bg-purple-50 text-[#6d28d9] py-4 rounded-2xl font-black uppercase tracking-widest text-xs"><LayoutDashboard size={18} /> Dashboard</button>
                  ...
                  <>
                    ...
                    <button onClick={() => navigate('/register')} className="w-full bg-[#6d28d9] text-white py-5 rounded-2xl font-black uppercase tracking-widest text-xs shadow-xl shadow-purple-100">Get Started</button>
                  </>
                )}
              </div>
            </motion.div>
...
const DropdownItem = ({ icon, label, onClick, variant = "default" }) => (
  <button onClick={onClick} className={`w-full flex items-center gap-3 px-4 py-3 text-sm font-black rounded-2xl transition-all ${variant === "danger" ? "text-red-500 hover:bg-red-50" : "text-slate-600 hover:bg-purple-50 hover:text-[#6d28d9]"
    }`}>
    {icon} {label}
  </button>
);