...
const fmt = (n, dec = 0) => new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: dec, maximumFractionDigits: dec }).format(n);
...
const STATE_RATES = {
    'California': 0.093, 'New York': 0.0685, 'Texas': 0,
    'Florida': 0, 'Washington': 0, 'Illinois': 0.0495,
};

function calcFederal(income) {
    return BRACKETS.reduce((tax, b) => income > b.min ? tax + (Math.min(income, b.max) - b.min) * b.rate : tax, 0);
}
...
export default function TaxEstimator() {
    ...
    const [form, setForm] = useState({
        country: 'United States', state: 'California', filingStatus: 'Single',
    ...
    const calculate = (e) => {
        ...
        const stdDeduction = form.filingStatus === 'Married Filing Jointly' ? 27700 : 13850;
        ...
        setResult({ gross, exp, netProfit, seTax, fedTax, stateTax, total, quarterly, effRate, taxable });
    };
                ...
                <div className="xl:col-span-2 card p-8">
                    ...
                    <form onSubmit={calculate} className="space-y-6">
                        <div className="grid grid-cols-2 gap-4">
                            <Sel label="Country/Region" opts={['United States', 'Canada', 'United Kingdom']} val={form.country} set={v => set('country', v)} />
                            <Sel label="State/Province" opts={['California', 'New York', 'Texas', 'Florida', 'Washington', 'Illinois']} val={form.state} set={v => set('state', v)} />
                        ...
                        {result ? (
                            <motion.div key="result" initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} className="flex flex-col h-full">
                                ...
                                <div className="rounded-2xl p-6 mb-6 text-center" style={{ background: '#0F172A' }}>
                                    ...
                                    <p className="text-[36px] font-black tracking-tight text-white">{fmt(result.quarterly, 2)}</p>
                                </div>

                                <div className="space-y-2.5 flex-1">
                                    <TaxLine label="Gross Income" val={fmt(result.gross)} />
                                    <TaxLine label="Net Profit" val={fmt(result.netProfit)} />
                                    <TaxLine label="Taxable Income" val={fmt(result.taxable)} />
                                    <div className="h-px my-3" style={{ background: '#F1F5F9' }} />
                                    <TaxLine label="SE Tax (15.3%)" val={fmt(result.seTax, 2)} red />
                                    <TaxLine label="Federal Income Tax" val={fmt(result.fedTax, 2)} red />
                                    <TaxLine label={`${form.state.split(' ')[0]} State Tax`} val={fmt(result.stateTax, 2)} red />
                                    <div className="h-px my-3" style={{ background: '#F1F5F9' }} />
                                    <TaxLine label="Annual Total" val={fmt(result.total, 2)} bold />
                                    <TaxLine label="Effective Rate" val={`${result.effRate.toFixed(1)}%`} bold />
                                </div>

                                <button
                                    onClick={() => { addTaxCalculation({ quarter: form.quarter, state: form.state, quarterlyPayment: result.quarterly, effectiveRate: result.effRate }); setSaved(true); }}
                                    disabled={saved}
                                    ...
                                    style={{ background: saved ? 'var(--income-light)' : 'var(--income)', color: saved ? 'var(--income)' : 'white', border: saved ? '1px solid #A7F3D0' : 'none', boxShadow: saved ? 'none' : '0 2px 12px rgba(16,185,129,0.3)' }}>
                                    <ShieldCheck size={16} /> {saved ? 'Saved!' : 'Save Calculation'}
                                </button>
                            </motion.div>
                        ) : (
